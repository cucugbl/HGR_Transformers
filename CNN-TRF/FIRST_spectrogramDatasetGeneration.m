% This code prepares the data for a machine learning model by dividing it into 
% training, validation, and test sets for transformer model creation, 
% generating spectrograms for gesture samples,
% as well as for "noGesture" samples.
%{
...
trainingDatastore = createDatastore('DatastoresTRF/training', categories);
validationDatastore = createDatastore('DatastoresTRF/validation', categories);
if Shared.includeTesting
    testingDatastore = createDatastore('DatastoresTRF/testing', categories);
end
% Clean up variables
clear categories
...
function [data, groundTruth] = generateFrames(signal, groundTruth, numGesturePoints, gestureName)
    ...
    numWindows = floor((length(signal)-Shared.FRAME_WINDOW) / Shared.WINDOW_STEP_TRF) + 1;
    ...
    for i = 1:numWindows
        ...
        traslation = ((i-1)* Shared.WINDOW_STEP_TRF);
        inicio = 1 + traslation;
        ...
        if totalOnes >= Shared.FRAME_WINDOW * Shared.TOLERANCE_WINDOW || ...
                totalOnes >= numGesturePoints * Shared.TOLERNCE_GESTURE_TRF
            isIncluded(i,1) = true;
...
function data = generateFramesNoGesture(signal, requestedWindows)
    ...
    if numWindowsFill > 0
        % Get a nogesture portion of the sample to use as filling
        filling = signal(1:numWindowsFill*Shared.WINDOW_STEP_TRF , :);
        % Fill before frame classification
        signal = [signal; filling];
    end
