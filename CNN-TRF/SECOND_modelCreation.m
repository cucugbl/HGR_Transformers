% modelCreation trains and saves the HGR CNN-TRF model.
% In this file is stablished the neural network architecture.
% Models are generated in "ModelsTRF/" folder.
...
dataDirTraining = fullfile('DatastoresTRF', 'training');
dataDirValidation = fullfile('DatastoresTRF', 'validation');
if Shared.includeTesting
    dataDirTesting = fullfile('DatastoresTRF', 'testing');
end
...
trainingDatastore = SpectrogramDatastoreTRF(dataDirTraining);
validationDatastore = SpectrogramDatastoreTRF(dataDirValidation);
if Shared.includeTesting
    testingDatastore = SpectrogramDatastoreTRF(dataDirTesting);
end
...
head=8;
channels=128;
lgraph = setNeuralNetworkArchitecture(inputSize, numClasses,head,channels);
analyzeNetwork(lgraph);
% Clean up variables
clear numClasses

%% THE OPTIONS ARE DIFINED
%gpuDevice(1); only if you need to use gpu
maxEpochs = 3; %10, 12 or 7
miniBatchSize = 64;%64
...
%% SAVE MODEL
save(['ModelsTRF/TransformerModel_PruebaNoValidaFinal_', datestr(now,'dd-mm-yyyy_HH-MM-ss')], 'net');

%% FUNCTION TO STABLISH THE NEURAL NETWORK ARCHITECTURE
function lgraph = setNeuralNetworkArchitecture(inputSize, numClasses,head,channels)
...
tempLayers = sinusoidalPositionEncodingLayer(22464,"Name","positionencode");
lgraph = addLayers(lgraph,tempLayers);

tempLayers = [
    additionLayer(2,"Name","addition")
    layerNormalizationLayer("Name","layernorm")
    selfAttentionLayer(head,channels,"Name","selfattention","NumValueChannels",128,"OutputSize",22464)
...
lgraph = connectLayers(lgraph,"sequence","Inception_1a-3x3_reduce");
lgraph = connectLayers(lgraph,"sequence","Inception_1a-1x1");
lgraph = connectLayers(lgraph,"sequence","Inception_1a-pool");
lgraph = connectLayers(lgraph,"sequence","Inception_1a-5x5_reduce");
...
lgraph = connectLayers(lgraph,"Inception_1e-pool_proj","depthcat_1e/in2");
lgraph = connectLayers(lgraph,"Inception_1e-1x1","depthcat_1e/in4");
lgraph = connectLayers(lgraph,"Inception_1e-5x5","depthcat_1e/in3");
lgraph = connectLayers(lgraph,"Inception_1e-3x3","depthcat_1e/in1");
...
lgraph = connectLayers(lgraph,"Inception_1f-3x3_relu","depthcat_1f/in3");
lgraph = connectLayers(lgraph,"Inception_1f-relu-pool_proj","depthcat_1f/in1");
lgraph = connectLayers(lgraph,"Inception_1f-1x1_relu","depthcat_1f/in4");
lgraph = connectLayers(lgraph,"Inception_1f-5x5_relu","depthcat_1f/in2");
